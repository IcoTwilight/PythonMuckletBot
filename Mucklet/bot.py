...
if __name__ == "__main__":
	test_bot = Bot("POZKFTOV4NQAUUVXQGI73MFFY5TZMJFM",
	               ResClient("wss://api.test.mucklet.com",
	                         "https://test.mucklet.com",
	                         )
	               )
	
	
	@test_bot.on("say", "pose", "wakeup", "sleep", "leave", "arrive", "describe", "action", "ooc", "whisper", "message",
	             "warn", "mail", "address", "controlRequest", "travel", "summon", "join", "leadRequest",
	             "followRequest",
	             "follow", "stopFollow", "stopLead")
	def on_say(event: EventBase):
		print(f"Event: {event}")
