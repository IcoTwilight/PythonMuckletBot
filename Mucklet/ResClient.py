...
if __name__ == "__main__":
	# run tests
	log_info = Logger(format_color = (80, 80, 210), bold = False)
	log_error = Logger(format_color = (210, 80, 80), bold = False)
	log_success = Logger(format_color = (80, 210, 80), bold = False)
	
	# create a connection
	client = ResClient(host = "wss://api.test.mucklet.com", origin = "https://test.mucklet.com").connect()
	
	# create a request
	request = client.request("version", protocol = "1.2.1", notification = False).send()
	auth = client.request("auth", "auth", "authenticateBot", token = "POZKFTOV4NQAUUVXQGI73MFFY5TZMJFM").send()

	log_info(str(request.wait()))
	log_info(str(auth.wait()))
	
	bot = client.request("call", "core", "getBot").send()
	
	log_info(str(bot.wait()))
	
	client.cache.get("core.bot.cftiute9gbrsdhhvk50g", "char", "settings").update(dnd = True)
	print(client.cache.get("core.bot.cftiute9gbrsdhhvk50g", "char"))
	
	client.emulate(event = "core.char.cftiute9gbrsdhhvk50g.owned.change",
	               data = {"controller": "player", "desc": {"action": "delete"}})
